...
import static ml.scyye.dmping.utils.StringUtils.limitWords;
...
public class CommandManager {
    ...
    public void onShutdown(CommandEvent event) {
        if (!event.getUser().getId().equals(Scyye.id)){
            event.reply("You can't do that!");
            print(event.getUser().getEffectiveName()+ " Attempted to shutdown the bot, but wasn't cool enough!");
            return;
        }
        print(event.getUser().getEffectiveName()+ " Successfully shutdown the bot, they must be cool!");
    ...
    public void onVersion(CommandEvent event) {
        event.reply(Main.VERSION+(Main.BETA?"-beta":""));
    }
    ...
    public void onChangeLog(CommandEvent event) {
        ...
        builder
                .addField("1.0.0", limitWords("Added all functionality from root's dmPing bot (do /original-js to view the original code for dmPing by Tessy)", charLimit), false)
                ...
                .addField("3.0.0", limitWords("Implemented the blacklist. /blacklist-add <User> to blacklist someone. /blacklist-remove <User> also works.", charLimit), false)
                .addField("4.0.0", limitWords("Added the Anti-Delete Functionality. Where the bot caches messages, and prevents deletion of messages.", charLimit), false)
                ...
                .addField("4.2.0", limitWords("Added /changelog, /version, and /original-js allowing you to view the entire history of the bot.", charLimit), false)
                .addField("4.2.1", limitWords("Updated TOKEN (thanks <@460904913732370434> bitch), cleaned up the code a bit, and added a MemberHandler that automatically gives <@&1058256598084096070>", charLimit), false)
                ...
                .addField("4.5.0", limitWords("Changed all commands to use a new framework. Updated caching and fixed it to work properly. Removed root from the author list. Removed the Deleter added in 2.0, as it was obsolete.", charLimit), false)
                ...
                .addField("5.2.0", limitWords("Made changelog more readable; removed obsolete versions. Reworked media. /help command is coming soon.", charLimit), false)
                .addField("5.3.0", limitWords("Changed deleted messages to webhooks. Changed the way the version system works. Started reworking caching.", charLimit), false)
        ;

        builder
                .setAuthor("Scyye")
                .setColor(Color.BLUE)
                .setThumbnail(Scyye.user.getAvatarUrl())
                .setTitle("**Changelog**");
