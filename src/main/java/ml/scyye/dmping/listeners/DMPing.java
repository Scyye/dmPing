...
public class DMPing extends S2AListener {
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		if (Arrays.stream(config.getBlacklist()).toList().contains(event.getAuthor().getId())) {
		...
		if (event.getAuthor().isBot()  || Arrays.stream(config.getBlacklist()).toList().contains(event.getAuthor().getId())) {
			return;
		}
